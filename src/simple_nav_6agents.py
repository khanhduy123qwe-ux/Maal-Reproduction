...
class PlanRecognitionLSTM(nn.Module):
    def __init__(self, obs_dim, action_dim=5, hidden_dim=64, num_goals=2):
        super().__init__()
...
class DQNAgent:
    def __init__(self, obs_dim, action_dim, lr=1e-3, gamma=0.95, epsilon=0.1):
        self.model = nn.Sequential(
            nn.Linear(obs_dim + 2, 128), nn.ReLU(),   # +2 vì num_goals=2
...
def train_simple_nav_6agents(episodes=12000, beta=0.01, seed=42):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)

    env = simple_spread_v3.parallel_env(N=6, max_cycles=100, continuous_actions=False)
    
    # Lấy obs_dim động từ environment
    obs_dim = env.observation_space('agent_0').shape[0]
    num_goals = 2
    
    agent = DQNAgent(obs_dim=obs_dim, action_dim=5)
    lstm = PlanRecognitionLSTM(obs_dim=obs_dim, action_dim=5, num_goals=num_goals)

    rewards_history, pcrs, ptrs = [], [], []

    for ep in tqdm(range(episodes), desc=f"SimpleNav6 β={beta}"):
    ...
    # Lưu kết quả
    os.makedirs("results/simple_nav_6agents", exist_ok=True)
    os.makedirs("plots/simple_nav_6agents", exist_ok=True)
    df = pd.DataFrame({"reward": rewards_history, "pcr": pcrs, "ptr": ptrs})
    df.to_csv(f"results/simple_nav_6agents/results_beta{beta}.csv", index=False)
    ...
    plt.savefig(f"plots/simple_nav_6agents/plot_beta{beta}.png")
    plt.close()

    print(f" HOÀN THÀNH SimpleNav6 β={beta} | Reward: {np.mean(rewards_history[-1000:]):.3f} | PCR: {np.mean(pcrs[-1000:]):.3f} | PTR: {np.mean(ptrs[-1000:]):.3f}")

if __name__ == "__main__":
    for beta in [0.0, 0.01]:
        train_simple_nav_6agents(beta=beta)